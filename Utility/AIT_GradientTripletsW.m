function [gradQ, gradb] = AIT_GradientTripletsW(x, idTriplet, Q, b, flagNORM)
...
SSdd = zeros(nTriplet, 1);
DDdd = zeros(nTriplet, 1);
for ii = 1:nTriplet
    SSdd(ii) = SSlogXBsYB(:, ii)'*Q*SSlogXBsYB(:, ii);
    DDdd(ii) = DDlogXBsYB(:, ii)'*Q*DDlogXBsYB(:, ii);
end
